...
from pages.locators import my_account_locators
...
class CreateCustomerAccount(BasePage):
    ...
    def check_mandatory_fields_error_text(self, text):
        ...
        for field in customer_fields_list:
            assert field.text == text, "There is no error for at least one field"
    ...
    def check_confirmation_pwd_error_text(self, text):
        pwd_confirmation_field_error = self.get_text(loc.pwd_confirmation_field_error_loc)
        assert pwd_confirmation_field_error == text, "The error message text is wrong"
